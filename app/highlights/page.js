...
const HighlightsPage = () => {
  ...
  useEffect(() => {
    if (eventParam) {
      // Normalize function to remove spaces, punctuation and lowercase
      const normalize = (str) => str.toLowerCase().replace(/[\s\W_]+/g, '');

      const normalizedEventParam = normalize(decodeURIComponent(eventParam));

      // Find highlight matching eventParam with normalized comparison
      const matchedHighlight = highlights.find((highlight) => {
        if (typeof highlight.title === 'string') {
          return normalize(highlight.title) === normalizedEventParam;
        } else if (Array.isArray(highlight.title)) {
          return highlight.title.some(titleItem => normalize(titleItem) === normalizedEventParam);
        }
        return false;
      });

      if (matchedHighlight) {
        setActiveSection(matchedHighlight.title);
      }
    }
  }, [eventParam]);
